...
<head>
    <meta charset="UTF-8">
    <title>Farmer Ranking — NkwaLedger</title>
    <style>
        body {
...
<body>
    <h1>Farmer Ranking</h1>
    <p class="meta">Agent: {{ $agentName }}</p>
    <p class="meta">Period: {{ $from }} to {{ $to }}</p>
    <p class="meta">Sorted by: {{ $sort === 'net' ? 'Net' : 'Income' }}</p>
    ...
    @if (count($roster) === 0)
        <p>No farmers assigned to this agent.</p>
    @else
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($roster as $row)
                    <tr>
                        ...
                        <td class="amount">{{ number_format($row['net'] / 100, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
